...
export const Scheduler = () => {
  ...
  const getEventStyle = (type: Event['type']) => {
    switch (type) {
      case 'interview': return 'bg-[#FEFCE8] text-[#92400E] border-[#FEF08A]';
  ...
  return (
    <div className="flex-1 flex overflow-hidden bg-transparent">
      ...
      <div className="w-[300px] bg-[#FFFFF5] border-r border-[#FEF08A] flex flex-col shrink-0 p-8 overflow-y-auto">
        <div className="mb-10">
           <div className="flex items-center justify-between mb-6">
              <h2 className="text-[14px] font-semibold text-[#1F2937]">Planner</h2>
              <button className="p-2 transition-colors hover:bg-[#FEF08A] rounded-lg text-[#92400E]"><Plus size={18} /></button>
           </div>
           
           {/* Mini Calendar Mock */}
           <div className="bg-[#FFFDF0] border border-[#FEF08A] rounded-2xl p-4">
              ...
              <div className="grid grid-cols-7 gap-1 text-center">
                  {Array.from({ length: 31 }, (_, i) => (
                    <span key={i} className={clsx("text-[11px] py-1.5 rounded-lg cursor-pointer transition-colors", i+1 === 14 ? "bg-[#92400E] text-white font-bold" : "text-[#1C1917]/60 hover:bg-[#FEF08A]")}>
                      {i + 1}
                    </span>
                  ))}
              </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-transparent">
        {/* Calendar Header */}
        <div className="h-20 px-8 border-b border-[#FEF08A] bg-white flex items-center justify-between shrink-0">
           <div className="flex items-center gap-6">
              ...
              <div className="flex bg-[#FEFCE8] p-1 rounded-lg border border-[#FEF08A]">
                 <button className="px-4 py-1.5 bg-white shadow-sm border border-[#FEF08A] rounded-md text-[12px] font-bold text-[#92400E]">Week</button>
           ...
           <div className="flex items-center gap-3">
              ...
              <button className="px-5 py-2.5 bg-[#B45309] text-white rounded-lg text-[13px] font-bold hover:bg-[#78350F] transition-all flex items-center gap-2 shadow-lg shadow-amber-100/50">
        ...
        {/* Grid Container */}
        <div className="flex-1 overflow-y-auto no-scrollbar relative bg-[#FFFDF0]">
           <div className="grid grid-cols-[100px_1fr] h-full min-w-[800px]">
              {/* Time Column */}
              <div className="border-r border-[#FEF08A] bg-[#FFFDF0]">
                 <div className="h-12 border-b border-[#FEF08A]" /> {/* Empty corner */}
                  {hours.map(hour => (
                    <div key={hour} className="h-[100px] border-b border-[#FEF08A] p-4 text-right">
              ...
              <div className="flex flex-col">
                  {/* Day Headers */}
                  <div className="h-12 border-b border-[#FEF08A] bg-[#FFFDF0] grid grid-cols-5 sticky top-0 z-10">
                     {days.map(day => (
                       <div key={day} className="flex items-center justify-center border-r border-[#FEF08A] last:border-r-0">
                 ...
                 <div className="relative flex-1 grid grid-cols-5">
                    ...
                    {/* Example Events */}
                    <div className="absolute top-[120px] left-[2%] w-[16%] h-[80px] p-3 rounded-xl border-l-4 shadow-sm group hover:scale-[1.02] transition-all cursor-pointer z-10 bg-[#FEFCE8] text-[#92400E] border-[#B45309]">
                    ...
                    <div className="absolute top-[200px] left-[62%] w-[16%] h-[60px] flex items-center justify-center bg-white border-2 border-dashed border-[#FEF08A] rounded-xl text-[#92400E]/40 text-[10px] uppercase font-bold tracking-widest hover:border-[#92400E] hover:text-[#92400E] transition-all cursor-pointer">
        ...
        <div className="h-8 bg-[#F9FAFB] border-t border-[#E0E7FF] px-8 flex items-center gap-4">
           <div className="flex items-center gap-1 text-[10px] font-medium text-[#6B7280]">
              <div className="w-2 h-2 rounded-full bg-[#92400E]" /> Human Resources
           </div>
           <div className="flex items-center gap-1 text-[10px] font-medium text-[#6B7280]">
